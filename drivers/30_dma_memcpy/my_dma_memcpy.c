...
#include <linux/slab.h>          /* For mem alloc */
#include <linux/dmaengine.h>     /* For using  dma */
#include <linux/dma-mapping.h>   /* For using  dma mapping */

/* Meta Information */
MODULE_LICENSE("GPL");
MODULE_AUTHOR("LA");
MODULE_DESCRIPTION("A DMA example for copying data from RAM to RAM");

/* Add a cbk to wait for transfer */
...
static int __init my_init(void) {
    ...
    /* Request dma channel (use a mask for selection) */
    dma_cap_zero(mask);
    dma_cap_set(DMA_SLAVE | DMA_PRIVATE, mask); /* The NULL params are used for filetering */
    ...
    src_buf = dma_alloc_coherent(chan->device->dev, 1024, &src_addr, GFP_KERNEL); /* Makes sure the allocated memory is not cache but in DRAM; 
                                                                                    will return a physical address not a virtual adress*/
...
/**
 * @brief This function is called, when the module is removed the kernel
 */
