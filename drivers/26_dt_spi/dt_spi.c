...
struct my_adc {
	struct spi_device *client; /* Access device through this pointer */
};
...
static int my_adc_probe(struct spi_device *client) {
	...
	if(!indio_dev) {
		printk("dt_iio - Error! Outof memory\n");
		return -ENOMEM;
	}
	...
	if(ret < 0) {
		printk("dt_iio - Error! Failed to set the SPI Bus \n");
		return ret;	
	}

	/* Set the ADC to power up */
	...
	/* Save indio_dev as spi device data */
	spi_set_drvdata(client, indio_dev);
	
	return devm_iio_device_register(&client->dev, indio_dev);  /* With this call we create the sysfs files for our iio device */
}
