...
/* Static example of declaration of waitqueue */
DECLARE_WAIT_QUEUE_HEAD(wq1); 
/* Dynamic examoke if declaration of waitqueue */
static wait_queue_head_t wq2;

/**
 * @brief Function which will be executed by the threads
 *
 * @params wait_sel         selection of wait function
 */
int thread_function(void *wait_sel) {
    ...
    printk("waitqueue - Thread monitoring wq%d finished exection!\n", selection);
    return 0;
}
 ...
 */
 static ssize_t my_write(struct file *File, const char *user_buffer, size_t count, loff_t *offs) {
    ...
    char buffer[16];
    printk("waitque - write callback called\n");
    ...
    /* Convert string to long int (10 for decimal nums, 16 for hexa) */
...
static int __init ModuleInit(void) {
    ...
    printk("waitque: Init threads\n");

    /* Register devoce number */
    if(register_chrdev(MYMAJOR, "my_dev_nr", &fops) != 0) {
        printk("Error!\nCould not register device number!\n");
        return -1;
    }
    printk("waitque -  Device number registered\n");

    /* Init Thread 1 */
    kthread_1 = kthread_run(thread_function, &t1, "kthread_1");
    if(kthread_1 != NULL) {
        printk("waitqueue - Thread 1 was created and is runing now!\n");
    }
    else {
